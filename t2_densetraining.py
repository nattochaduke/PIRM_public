...
def main():
    ...
    parser.add_argument('--out', '-o', default='result_t2',
    ...
    flip = True if args.flip == "True" else False
    rotate = True if args.rotate == "True" else False
    ...
    model = models.ResidualDenseNetDeconv(scale=1.5, in_size=args.patchsize, in_channels=17,
                                            out_channels=14, n_feats=args.n_feats, n_RDBs=args.n_RDBs,
                                          n_denselayers=args.n_denselayers,
                                          growth_rate=args.growth_rate, res_scale=args.res_scale)
    ...
    train = datasets.T1Dataset(list(range(100)),
                               'data/PIRMt2/normalized/train_stack_reg.npy', 'data/PIRMt2/normalized/train_spec_hr.npy',
                               patchsize=args.patchsize, scale=1.5, train=True, image_concat=args.image_concat, mixup=args.mixup,
                               target_bands=bands, flip=flip, rotate=rotate)
    validation = datasets.T1Dataset(list(range(10)),
                                    'data/PIRMt2/normalized/val_stack_reg.npy', 'data/PIRMt2/normalized/val_spec_hr.npy',
                                    patchsize=64, scale=1.5, train=False, target_bands=bands)
    ...
    trainer.extend(extensions.PrintReport(
        ['epoch', 'main/MAE', 'main/MSE', 'main/MRAE', 'main/loss', 'main/SID',
         'validation/main/MAE', 'validation/main/MSE', 'validation/main/MRAE', 'validation/main/SID', 'validation/main/loss', 'elapsed_time']),
        trigger=(1, 'epoch'))
    trainer.extend(extensions.ProgressBar())
    trainer.extend(extensions.snapshot_object(model.predictor, 't2_denseresnet_{.updater.epoch}.npz'), trigger=(50, 'epoch'))
