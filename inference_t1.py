...
def main():
    parser = argparse.ArgumentParser(description='inference for track 1.')
    ...
    def test_inference(snapshot, ensemble=True, datafile='data/PIRMt1/normalized/testing_lr3_data.npy'):
        model = models.ResidualDenseNet(scale=3, in_channels=14, out_channels=14, n_feats=256,
                                        n_RDBs=20, n_denselayers=6, growth_rate=64,
                                        res_scale=1)
    ...
    '''.format(comment)
        ...
        for i in range(20):
            result[i].tofile(target_directory + '/image_{}_tr1.fla'.format(i + initial_number))
            with open(target_directory + '/image_{}_tr1.hdr'.format(i + initial_number), 'w') as f:
                f.write(headerfile)
    ...
    def pipeline(snapshot, target_directory, ensemble=True, mode="test"):
        if mode == "test":
            datafile = 'data/PIRMt1/normalized/testing_lr3_data.npy'
        elif mode == "val":
            datafile = 'data/PIRMt1/normalized/val_lr3.npy'
    ...
    pipeline(args.model, args.target, ensemble=True, mode="test")
